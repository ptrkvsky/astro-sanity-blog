...
isHome: true
slug: 'react-fonction-retour'
title: React fonction de retour et useEffect
metaTitle: React fonction de retour et useEffect
metaDescription: Tout savoir sur la fonction de nettoyage à utiliser dnas le hook useEffect. Elle pourra vous être utile dans bien des situations, notamment si vous rencontrez l'erreur suivante - Warning  Can't perform a React state update on an unmounted component. This is a no-op, but it indicates a memory leak in your application. To fix, cancel all subscriptions and asynchronous tasks in the componentWillUnmount method.
---
